...
light.theme <- theme_light(base_size = 18) +
...
#Compute mean
mean2016 <- as.data.frame(aggregate(cbind(Temperature, Wind.Speed, u.wind, v.wind) ~
                                      Date.noon, wind2016, mean))
mean2016$Wind.direction.avg <- (atan2(mean2016$u.wind, mean2016$v.wind) * 360/2/pi)
+ 180
...
#Compute mean
mean2017 <- as.data.frame(aggregate(cbind(Temperature, Wind.Speed, u.wind, v.wind) ~
                                      Date.noon, wind2017, mean))
...
#Compute mean
mean2018 <- as.data.frame(aggregate(cbind(Temperature, Wind.Speed, u.wind, v.wind) ~
                                      Date.noon, wind2018, mean))
...
data.gather <- gather(data.all, "Site.Name", "Concentration", 
                     Chad.Baker:Ochsner.Hospital) %>% na.exclude()

Temp.auto <- lme(data = data.gather, Concentration ~ Temperature * Site.Name, 
                 random = ~1|Week)
Temp.auto
ACF(Temp.auto)
Temp.mixed <- lme(data = data.gather, Concentration ~ Temperature * Site.Name, 
                 random = ~1|Week, correlation 
                 = corAR1(value = -0.066), method = "REML")
summary(Temp.mixed)

Speed.auto <- lme(data = data.gather, Concentration ~ Wind.Speed * Site.Name, 
                 random = ~1|Week)
Speed.auto
ACF(Speed.auto)
Speed.mixed <- lme(data = data.gather, Concentration ~ Wind.Speed * Site.Name, 
                 random = ~1|Week, correlation 
                 = corAR1(value = -0.054), method = "REML")
summary(Speed.mixed)

Speed.fixed <- gls(data = data.gather, Concentration ~ Wind.Speed * Site.Name, 
                   method = "REML")
summary(Speed.fixed)
anova(Speed.mixed, Speed.fixed)

# Plot
speed.plot <-
  ggplot(data.gather, aes(x = Wind.Speed, y = Concentration, 
                          color = Site.Name, shape = Site.Name)) +
  geom_point() +
  geom_abline(intercept = 11.562, slope = -1.664) +
  scale_color_brewer(palette = 'RdYlBu', name = "Site Name",
                     labels = c("Chad Baker St", "Elementary School", 
                                "East Saint John High School", "Hwy 44",
                                "Mississippi River Levee", "Ochsner Hospital")) +
  scale_shape_discrete(labels = c("Chad Baker St", "Elementary School", 
                                "East Saint John High School", "Hwy 44",
                                "Mississippi River Levee", "Ochsner Hospital"),
                       name = "Site Name") +
  ylab(expression(paste("Chloroprene Concentration (", mu, "g/", m^3,")", separate = ""))) +
  xlab('Wind Speed, mph') +
